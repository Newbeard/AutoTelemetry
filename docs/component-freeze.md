...
| Vehicle-path controller | `LM74502QDDFRQ1` | Texas Instruments | AEC-Q100 grade 1 | 8-pin thin SOT-23 | `PROPOSED CHANGE`, approved for Task 5 | 3.2–65 V, −65 V reverse withstand, 110 µA maximum operating current, programmable UV/OV and 60 µA gate-source current. Unlike the H variant, TI supports external `Cdvdt` inrush control; 2 A gate sink preserves rapid turn-off. It does **not** reverse-block while on, so UVLO must open above the USB source crossover. | Earlier `LM74502HQDDFRQ1` is rejected because its fast turn-on is not compatible with TI's illustrated inrush network; LM74700-Q1 lacks OV disconnect. | High architecture; exact thresholds/inrush provisional |
...
| Input load-dump TVS | `SM8SF24CA-Q` | Bourns | AEC-Q101 | 8.1 × 10.5 × 1.3 mm DFN | `FROZEN` | Bidirectional part permits −14 V reverse battery without forward-biasing the clamp; 24 V stand-off, 26.7–29.5 V breakdown, 38.9 V maximum clamp at 180 A, 7 kW 10/1000 µs, 10 µA maximum leakage at 24 V/25 °C. Physical clamp/energy validation remains mandatory. | Vishay `SM8S24CA` credible larger alternate; ST `LDP01-28AY` rejected at pre-controller position because it is unidirectional; 33 V parts lose MOSFET margin. | High part/architecture; test evidence pending |
| Input EMI filter | 2.2–4.7 µH, ≥3 A/≥4 A saturation post-switch damped C-L-C envelope | exact order codes TBD | Automotive grade/AEC-Q200 where applicable | — | `FROZEN topology`, passives `PROVISIONAL` | Raw 100 nF + 1 µF/100 V; post-switch 100 nF + 1 µF/50 V, series inductor, 2×4.7 µF/50 V plus 47–100 µF/50 V and an R-C damping option. Exact impedance, ESR, bias derating, inrush and converter interaction require Task 5 calculation/measurement. | Undamped ceramic π filter rejected; ferrite-only option retained only if EMI testing proves sufficient. | High topology, low exact passive confidence |
...
The approved Task 4.7 envelope is 6–18 V operation, +26 V/60 s jump-start survival by disconnect, −14 V/60 s reverse survival, and +38 V suppressed-load-dump source survival by disconnect. `VEHICLE_PROTECTED` must remain ≤24 V in the approved test matrix. Severe unsuppressed public reference conditions remain outside the v1 guarantee. See [`automotive-electrical-profile.md`](automotive-electrical-profile.md) and [`input-protection-architecture.md`](input-protection-architecture.md) for source-to-clamp calculations and the explicit test/compliance boundary.

UV falling is 6.0 V nominal and OV is 18.0 V nominal. Task 5 must calculate threshold/divider tolerance, guarantee steady OV cutoff ≤20 V, guarantee UV cutoff above the USB crossover, and calculate `Cdvdt` inrush. Brownout uses controlled reset; full crank ride-through is not required. Exact filter/inrush passives remain provisional.
